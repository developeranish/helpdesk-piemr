...
//  Colors  as  they  are  defined  in  HTML  3.2 
            ...
            "platinum"=>array( "red"=>0xC0,  "green"=>0xC0,  "blue"=>0xC0), 
...
//  Additional  colors  as  they  are  used  by  Netscape  and  IE 
            ...
            "rose"=>array( "red"=>0xB7,  "green"=>0x7B,  "blue"=>0x82), 
   ...
   function create_color($colors=''){
   		$colors = explode(' ', $colors);
   		$return = 0 ;
 		foreach ($colors as $color ) { 
 			$s[] = explode(',',GetColor($color));
   		}
   		$v1 = $v2 = $v3 = 0;
   		for($i=0;$i<count($s);$i++){
   			$v1 =  $v1+$s[$i][0]; 
   		...
   		return $sx = sprintf("#%02x%02x%02x",round($v1/2),round($v2/2),round($v3/2)); 
   }